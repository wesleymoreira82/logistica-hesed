...
# 2. Entrada de Dados (Sem st.form para desativar envio por Enter)
...
# 3. Campos de Endereço Separados
...
if not pedido or not nome or not cep_input or not numero or st.session_state.erro_cep:
    st.warning("Preencha corretamente os campos obrigatórios (Pedido, Nome, CEP e Número) para liberar a confirmação.")
else:
    # Montagem do resumo formatado em linhas
    ...
    # Botão de Envio Único (Ignora o Enter)
    if st.button("Confirmar e Enviar para a Loja"):
        try:
            # Puxa os dados das Secrets do Streamlit Cloud
            creds_info = dict(st.secrets["gcp_service_account"])
            
            # Limpeza crucial para evitar o erro de 'Invalid JWT Signature/PEM file'
            ...
            # Abre a planilha pelo nome exato e envia para a Coluna A
            sheet = client.open("Logistica_Hesed").sheet1
            sheet.append_row([resumo_texto])
            
            st.success("Dados enviados com sucesso para a planilha da Loja Hesed!")
            st.balloons()
